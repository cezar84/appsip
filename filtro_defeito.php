...
$query = sprintf("SELECT protocolo,defeito,bairro,logradouro,numposte, data_notf,usuario FROM tbl_defeito ORDER BY bairro");
$dados = mysqli_query($con,$query ) or die(mysql_error());
$linha = mysqli_fetch_assoc($dados);
// calcula quantos dados retornaram
$total = mysqli_num_rows($dados);
...
$html[$i] = "";
    $html[$i] .= "<table>";
    $html[$i] .= "<tr>";
    $html[$i] .= "<td><b>Protocolo</b></td>";
	$html[$i] .= "<td><b>Defeito</b></td>";
	$html[$i] .= "<td><b>Bairro</b></td>";
	$html[$i] .= "<td><b>Logradouro</b></td>";
	$html[$i] .= "<td><b>Nº poste</b></td>";
    $html[$i] .= "<td><b>Data</b></td>";
	$html[$i] .= "<td><b>Usuário</b></td>";
    $html[$i] .= "</tr>";
    $html[$i] .= "</table>";
}
...
$i = 1;
while($linha = mysqli_fetch_assoc($dados));{
 $retorno_protocolo = $ret['protocolo'];
 $retorno_defeito = $ret['defeito'];
 $retorno_bairro = $ret['bairro'];
 $retorno_logradouro = $ret['logradouro'];
 $retorno_numposte = $ret['numposte'];
 $retorno_data_notif = $ret['data_notif'];
 $retorno_usuario = $ret['usuario'];

    $html[$i] .= "<table>";
    $html[$i] .= "<tr>";
    $html[$i] .= "<td> $retorno_protocolo</td>";
	$html[$i] .= "<td> $retorno_defeito</td>";
	$html[$i] .= "<td>$retorno_bairro</td>";
	$html[$i] .= "<td>$retorno_logradouro</td>";
	$html[$i] .= "<td>$retorno_numposte</td>";
    $html[$i] .= "<td> $retorno_data_notif</td>";
	 $html[$i] .= "<td>  $retorno_usuario</td>";
    $html[$i] .= "</tr>";
    $html[$i] .= "</table>";
    $i++;
}
 
$arquivo = 'relatorio_defeito.xls';
